 ...
 * 我的账单(易宝)
 */
public class MeMyBillFragment extends BaseFragment implements LoadingView {
...
//                remove("MeMyBillFragment");
    ...
    private void initData() {
        ...
        billFragmentPresenter.loadData(strn);
    }
...
//                    if (list.size() > 0){
    ...
    List<ScreensModel> billStatusQuery;
    ...
    private void setPicker(ScreenModel model) {
        billStatusQuery = model.billStatusQuery;
        billDateQuery = model.billDateQuery;
        for (int i = 0; i < billStatusQuery.size(); i++) {
            option1.add(billStatusQuery.get(i).title);
    ...
    public void on2OptionPicker(View view) {
        ...
        picker.setOnWheelListener(new WheelPicker.OnWheelListener<int[]>() {
            ...
            public void onSubmit(int[] result) {
                ...
                typeInfo = billStatusQuery.get(result[0]).status;
