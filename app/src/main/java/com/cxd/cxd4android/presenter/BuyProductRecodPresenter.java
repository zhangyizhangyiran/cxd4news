 ...
 * on 2015/9/23.
 */
public class BuyProductRecodPresenter extends MicroBasePresenter<LoadingView> {

    public BuyProductRecodPresenter(LoadingView view) {
        attachView(view);
    }
    ...
    public void loadData(String strn) {
        ...
        addSubscription(apiStores.getInvestRecords(strn),
                new SubscriberCallBack<ProductRecodBaseModel>(new ApiCallback<ProductRecodBaseModel>() {
                    @Override
                    public void onSuccess(ProductRecodBaseModel model) {
