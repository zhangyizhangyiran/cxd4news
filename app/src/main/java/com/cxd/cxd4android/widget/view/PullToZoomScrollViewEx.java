...
public class PullToZoomScrollViewEx extends PullToZoomBase<ScrollView> {
    ...
    private View scrollYs;

    final float scale = getResources().getDisplayMetrics().density;
    int padding_in_px = (int) (190 * scale + 0.5f);
        ...
        ((InternalScrollView) mRootView).setOnScrollViewChangedListener(new OnScrollViewChangedListener() {
            ...
            public void onInternalScrollChanged(int left, int top, int oldLeft, int oldTop) {
                if (isPullToZoomEnabled() && isParallax()) {
                    int f = mHeaderHeight - mHeaderContainer.getBottom() + mRootView.getScrollY();
                   /* if (f>padding_in_px){

                        getScrollYs();
                    }else if (f<=padding_in_px){
                        getScrollYs2();
                    }*/
...
/*    public void setScrollYs(View tb){
        scrollYs=tb;
    }
    //当滑动距离大于300的时候设置为这个颜色
    public View getScrollYs(){
        scrollYs.setBackgroundColor(Color.parseColor("#F0F0F0"));
        return scrollYs;
    }
    //当滑动距离小于300的时候设置为这个颜色
    public View getScrollYs2(){
        scrollYs.setBackgroundColor(Color.parseColor("#00000000"));
    ...
    protected void smoothScrollToTop() {
        mScalingRunnable.startAnimation(300L);
    }
