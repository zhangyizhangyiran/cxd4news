...
public class RolloutBaseActivity extends Activity {
    ...
    protected void setShowimage() {
        if (mSpring.getEndValue() == 0) {
            ...
            tx = Width / 2 - (bdInfo.x + 10 + img_w / 2);
    ...
    private void MoveBackView() {
        ...
        AnimatorSet set = new AnimatorSet();
        set.playTogether(
                ObjectAnimator.ofFloat(showimg, "translationX", 1).setDuration(200),
                ObjectAnimator.ofFloat(showimg, "translationY", 1).setDuration(200)
        );
        set.addListener(new Animator.AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animator) {
