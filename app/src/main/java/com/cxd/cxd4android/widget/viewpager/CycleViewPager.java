...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
    ...
    public void setData(List<ImageView> views, List<BannerModel> list, ImageCycleViewListener listener) {
     ...
     */
    public void setDataScalType(List<ImageView> views, List<BannerModel> list, ImageCycleViewListener listener) {
        int showPosition = 0;
        ...
        adapter = new ViewPagerAdapter(1);//设置为1时，ViewPagerAdapter中执行相应的设置

        // 默认指向第一项，下方viewPager.setCurrentItem将触发重新计算指示器指向
        if (list.size() > 1) {/** Modify By Gele **/
            setIndicator(0);
        }
     ...
     * @param showPosition 默认显示位置
     */
    private void setData(List<ImageView> views, List<BannerModel> list, ImageCycleViewListener listener, int showPosition) {
        mImageCycleViewListener = listener;
        this.list = list;
        this.imageViews.clear();
    ...
    private class ViewPagerAdapter extends PagerAdapter {
        ...
        private ViewPagerAdapter(int type) {
            this.type = type;
        }
        ...
        public View instantiateItem(ViewGroup container, final int position) {
            ...
            if (type == 1) {
                v.setAdjustViewBounds(true);//设置图片按比例缩放
                v.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
                //v.setImageResource(R.mipmap.gift_confirm_bg);
            }
            if (mImageCycleViewListener != null) {
                v.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (list.size() > 1) {/** Modify By Gele **/
                            mImageCycleViewListener.onImageClick(list.get(currentPosition - 1), currentPosition, v);
                        } else if (list.size() == 1) {
                            mImageCycleViewListener.onImageClick(list.get(currentPosition), currentPosition, v);
                        }
    ...
    public void onPageScrollStateChanged(int arg0) {
        ...
        EventBus.getDefault().post(new ISwipeRefreshCallBack(arg0 == ViewPager.SCROLL_STATE_IDLE));
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].setBackgroundResource(R.mipmap.ic_bout_pointalph);
        }
        if (indicators.length > selectedPosition)
            indicators[selectedPosition].setBackgroundResource(R.mipmap.ic_bout_point);
    }
    ...
    public static interface ImageCycleViewListener {
         ...
         * @param imageView
         */
        public void onImageClick(BannerModel info, int postion, View imageView);
    }
}